...
if (!("animation" in style)) {
    for (var styleName in animationend) {
        if (styleName in style) {
            exports.prefixedAnimationEnd = prefixedAnimationEnd = animationend[styleName];
            break;
        }
    }
    for (var _styleName in animationiteration) {
        if (_styleName in style) {
            prefixedAnimationIteration = animationiteration[_styleName];
            break;
        }
    }
}
...
if (!("transition" in style)) {
    for (var _styleName2 in transitionend) {
        if (_styleName2 in style) {
            exports.prefixedTransitionEnd = prefixedTransitionEnd = transitionend[_styleName2];
            break;
        }
    }
}
    ...
    eachEl(el, function (targ) {
        var handler = function handler(ev) {
            ...
            if (ev.target === targ) {
                callback(targ);
                if (once) {
                    targ.removeEventListener(eventName, handler, false);
                }
            }
        };
...
var AnimationUtil = {
    ...
    */
    makeKnown: function makeKnown(animName) {
    ...
    */
    animate: function animate(el, animName, callback, extraClass, persistClasses) {
        knownClasses[animName] = true;
        eachEl(el, function (targ) {
            var handler = function handler(ev) {
                if (!persistClasses) {
    ...
    * @param bool True if you want the classes to persist after completion, otherwise they are removed by default
    */
    animateOutIn: function animateOutIn(el, animOutName, animInName, outCallback, inCallback, extraClass, persistClasses) {
        var _this = this;

        this.animate(el, animOutName, function (targ) {
            if (outCallback) {
                outCallback(targ);
            }
            _this.animate(targ, animInName, inCallback, extraClass, persistClasses);
    ...
    * @param bool True if you want the classes to persist after completion, otherwise they are removed by default
    */
    fadeOutIn: function fadeOutIn(el, outCallback, inCallback, extraClass, persistClasses) {
    ...
    */
    onAnimationEnd: function onAnimationEnd(el, once, callback) {
    ...
    * @param function callback
    */
    onAnimationIteration: function onAnimationIteration(el, once, callback) {
    ...
    * @param function callback 
    */
    onTransitionEnd: function onTransitionEnd(el, once, callback) {
