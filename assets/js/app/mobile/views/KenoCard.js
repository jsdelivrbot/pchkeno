...
import ChoicesViewMobile from "app/mobile/views/Choices";
import ButtonView from "app/views/Button";
import OneADay from "app/modules/OneADay";

//Delay for idle message
const IDLE_MESSAGE_SECONDS = 30000;

//how long to idle message for -
const IDLE_MESSAGE_SHOW_FOR = 5000;

const KenoCardViewMobile = function () {};

Object.assign( KenoCardViewMobile.prototype, KenoCardView.prototype, {
	EVENTS: {
		SUBMIT: `submit`,
	...
	init: function ( appModel ) {
		this.showIntroAnims = true;
		this._onCountdownTick = this.onCountdownTick.bind( this );
		this.selectedNumbers = new Set();
		this.appModel = appModel;
		this.countdownView = new KenoCountdownView();
		this.choicesView = new ChoicesViewMobile();
		this.coaching = new CoachingLightBox( appModel.coachingEnabled );
	...
	mount: function ( ctx ) {
		...
		this.cardRootNode = $( `.card__form` );
		this.cardControls = $( `.card__controls` );
		...
		this.submitBtn = this.cardControls.find( `.card__controls__btn--submit` );
		this.submitBtn.on( `click`, this.onSubmit.bind( this ));
		this.controlBarNumbers = $( `.control-bar__your-numbers ul` );
		// this.numToSelect = parseInt(this.rootNode.attr("data-numbers"), 10);
		this.numToSelect = 10;
		...
		this.mountControlBar( this.controlBar );

		this.idleMessageShowing = false;
		this.coachingScreensShowing = false;
		this.uniNavShowing = false;

		//close coachingscreens when uni-nav is opened
		$(
			`.uninav__burger, .uninav__multi-nav--tokens, .uninav__multi-nav--level`
		).on( `click touchstart`, () => {
			this.toggleCoachingScreens();
			this.uniNavShowing = !this.uniNavShowing; //toggle uniNavShowing
		});

		if ( !( this.choicesView && this.choicesView.isShown())) {
			this.idleTimeout = this.setIdleTimeout();

			if ( this.showIntroAnims ) {
		...
		this.rootNode.on( `touchstart`, () => {
			this.clearIdleTimeout();
		});

		this.controlBar.on( `touchstart`, () => {
			this.clearIdleTimeout();
		});
	},
	...
	setIdleTimeout: function () {
		return window.setTimeout(() => {
			if ( !this.uniNavShowing ) {
				//hide idle message when uni-nav is showing so it doesn't overlap or cause coaching screen issues
	...
	clearIdleTimeout: function () {
		...
		this.idleMessageShowing = false;
		this.coaching.showLightbox();
		clearTimeout( this.idleTimeout );
		this.idleTimeout = this.setIdleTimeout();
	},
	toggleCoachingScreens: function () {
		if ( !this.idleMessageShowing ) {
			this.coaching.toggleCoachingScreens();
		}
	},
	mountControlBar: function ( controlBar ) {
		$( `body` ).append( controlBar );
		$( `#page` )
			.find( this.controlBar )
			.remove();
		controlBar.css( `visibility`, `visible` );
	},
	unmountControlBar: function ( controlBar ) {
		if ( controlBar ) {
			controlBar.remove();
		}
	},
	unmount: function ( ctx ) {
		...
		this.unmountControlBar( this.controlBar );
		this.idleMessageShowing = false;
		this.coaching.hideLightbox();
		//  coaching.hide();
	},

	handleCoachingScreensOnNumberClicked: function () {},
	onNumberClicked: function ( ev ) {
		...
		if ( this.selectedNumbers.size >= this.numToSelect ) {
			...
			this.coaching.show(
				...
				coachingMessages.default.class,
				`body`
			);
		}
	},
	onQuickPick: function ( ev ) {
		...
		for ( let i = 0; i < this.numToSelect; i++ ) {
			...
			this.addPickedNumberToControlBar( num );
			this.cardRootNode
	...
	startAnims: function () {
		...
		const handleAnimationEnd = function ( event ) {
			if ( event.animationName === `zoomOut` ) {
				...
				// Coaching screens
				if ( that.appModel.completedCardNumber === 0 && !that.isShownChoices()) {
					that.coaching.show(
						coachingMessages.default.msg,
						coachingMessages.default.top,
						coachingMessages.default.left,
	...
	onClear: function ( ev ) {
		...
		this.selectedNumbers.clear();
		this.removeAllNumbersFromControlBar();
	...
	}, //placeholder for now until GA tags story, might be able to reuse function in desktop view
	...
	handleNumberClicked: function ( ev ) {
		...
		if ( this.selectedNumbers.has( num )) {
			...
			this.removePickedNumberFromControlBar( num );
	...
	addPickedNumberToControlBar: function ( num ) {
		this.controlBarNumbers.append( `<li>` + num + `</li>` );
	},
	removePickedNumberFromControlBar: function ( num ) {
		const numbers = this.controlBarNumbers.find( `li` );

		numbers.forEach( number => {
			if ( parseInt( number.innerHTML, 10 ) === num ) {
				number.remove();
			}
		});
	},
	removeAllNumbersFromControlBar: function () {
		this.controlBarNumbers.html( `` );
	},
	...
	showEntryConfirmedCircle: function ( imgPath ) {
		const promise = new Promise( resolve => {
			const entryConfirmedCircle = $(
				renderTemplate( `shared/mobile-promotion-circle-entry.html`, {
					imgPath: imgPath
				})
			);
			...
			$( `body` )
				.addClass( `noScroll` )
				.append( entryConfirmedCircle[0]);
			document.querySelector( `.wrapperEntry` ).classList.add( `display` );
			setTimeout(() => {
				document.querySelector( `.wrapperEntry` ).classList.remove( `display` );
	...
	disableIntroAnims: function () {
		this.showIntroAnims = false;
	}
});

export default KenoCardViewMobile;
