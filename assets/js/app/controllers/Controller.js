...
class Controller {
	...
	route ( ctx, next ) {
		...
		this.isActive = true;
		if ( this.view ) {
			this.appModel.currentURL = this.routedUrl;
			this.view.mount( ctx );
			ctx.handled = true;
		}
		next();
	...
	showTimeoutError () {
		if ( !$timeoutError ) {
			$timeoutError = $( renderTemplate( `shared/kenocard-oops-timeout.html` ));
			$timeoutError.appendTo( document.body );
		}
		$timeoutError.modal({ backdrop: false });
	}
}
