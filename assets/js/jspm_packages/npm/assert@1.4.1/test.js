...
(function(Buffer, process) {
	...
	function tests(assert, what) {
		...
		test('assert.equal', function() {
			...
			assert.doesNotThrow(
				makeBlock(assert.equal, undefined, undefined),
				'equal'
			);
		...
		test('assert.deepEqual - ES6 primitives', function() {
			assert.throws(
				makeBlock(assert.deepEqual, null, {}),
				assert.AssertionError
			);
			assert.throws(
			...
			assert.throws(
				makeBlock(assert.deepEqual, true, {}),
				assert.AssertionError
			);
		...
		test('assert - Ensure reflexivity of deepEqual with `arguments` objects', function() {
			...
			assert.throws(
				makeBlock(assert.deepEqual, [], args),
				assert.AssertionError
			);
			assert.throws(
				makeBlock(assert.deepEqual, args, []),
