...
(function(process) {
	...
	var File = (function(_Store) {
		...
		File.prototype.addHelper = function addHelper(name) {
			...
			var uid = (this.declarations[name] = this.scope.generateUidIdentifier(
				name
			));
		...
		File.prototype.addTemplateObject = function addTemplateObject(
			...
			var name =
				helperName + '_' + raw.elements.length + '_' + stringIds.join(',');
			var declar = this.declarations[name];
			if (declar) return declar;
			var uid = (this.declarations[name] = this.scope.generateUidIdentifier(
				'templateObject'
		...
		File.prototype.mergeSourceMap = function mergeSourceMap(map) {
			...
			if (inputMap) {
				var _ret = (function() {
					var inputMapConsumer = new _sourceMap2.default.SourceMapConsumer(
						inputMap
					);
					var outputMapConsumer = new _sourceMap2.default.SourceMapConsumer(
						map
					);
			...
			if (parserOpts) {
				...
				if (parserOpts.parser) {
					if (typeof parserOpts.parser === 'string') {
						var dirname =
							_path2.default.dirname(this.opts.filename) || process.cwd();
